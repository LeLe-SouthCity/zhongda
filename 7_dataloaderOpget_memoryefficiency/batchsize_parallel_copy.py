...
def scatter(inputs, target_gpus, chunk_sizes, dim=0):
    ...
    def scatter_map(obj):
        if isinstance(obj, torch.Tensor):
            try:
                # print('Scatter.apply(target_gpus, chunk_sizes, dim, obj) :',Scatter.apply(target_gpus, chunk_sizes, dim, obj))
                # 返回值是一个数组（矩阵）
...
class BalancedDataParallel(DataParallel):
    def __init__(self, gpu0_bsz, gpu1_bsz, *args, **kwargs):
        self.gpu0_bsz = gpu0_bsz
        self.gpu1_bsz = gpu1_bsz
